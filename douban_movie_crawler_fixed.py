    ...
    def parse_movie_list(self, html):
        ...
        for item in movie_items:
            try:
                ...
                if hd_div:
                    link_element = hd_div.find('a')
                    if link_element:
                        movie_url = link_element.get('href')
                        title_element = link_element.find('span', class_='title')
                        if title_element:
                            ...
                            print(f"解析到电影: {movie_title}")
            except Exception as e:
                print(f"解析电影链接失败: {e}")
                continue
                
        print(f"成功解析 {len(movie_links)} 个电影链接")
        return movie_links
...
def main():
    ...
    # 创建爬虫实例
    crawler = DoubanMovieCrawlerFixed()
